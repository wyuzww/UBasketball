            ...
            public void handleMessage(Message msg) {
                ...
                if (result == SMSSDK.RESULT_COMPLETE) { //回调完成
                    if (event == SMSSDK.EVENT_SUBMIT_VERIFICATION_CODE) { //验证验证码成功
                        ...
                        Toast.makeText(ResetPasswordActivity.this, "返回支持发送验证码的国家列表", Toast.LENGTH_SHORT).show();
    ...
    private void SendVerificationCode() {
        ...
        if (!phoneNumber_ET.getText().toString().trim().equals("")) {
            if (checkTel(phoneNumber_ET.getText().toString().trim())) {
                ...
                timeCount.start();
                Toast.makeText(ResetPasswordActivity.this, "已发送验证码", Toast.LENGTH_SHORT).show();
            } else {
                Toast.makeText(ResetPasswordActivity.this, "请输入正确的手机号码", Toast.LENGTH_SHORT).show();
            }
        } else {
            Toast.makeText(ResetPasswordActivity.this, "请输入手机号码", Toast.LENGTH_SHORT).show();
        }
    }
            ...
            public void onFailure(Call call, IOException e) {
                if (e instanceof SocketTimeoutException) {
                    runOnUiThread(new Runnable() {
                        @Override
                        public void run() {
                            Toast.makeText(ResetPasswordActivity.this, "连接超时", Toast.LENGTH_SHORT).show();
                    ...
                    runOnUiThread(new Runnable() {
                        @Override
                        public void run() {
                            Toast.makeText(ResetPasswordActivity.this, "无法连接到服务器", Toast.LENGTH_SHORT).show();
                            register_Dialog.dismiss();
                        }
                    });
                } else {
                    runOnUiThread(new Runnable() {
                        @Override
                        public void run() {
                            Toast.makeText(ResetPasswordActivity.this, "网络出错", Toast.LENGTH_SHORT).show();
                            register_Dialog.dismiss();
                        }
                    });
                }
            ...
            public void onResponse(Call call, Response response) throws IOException {
                if (response.code() == 200) {
                    ...
                    runOnUiThread(new Runnable() {
                        @Override
                        public void run() {
                            Toast.makeText(ResetPasswordActivity.this, "连接到服务器错误", Toast.LENGTH_SHORT).show();
                            register_Dialog.dismiss();
                        }
                    });
                }
